...
def identify_critical_regions(coupling_map, coordination_map, thresholds=None):
    ...
    coupling_map_clean = np.where(valid, coupling_map, 0)
    coordination_map_clean = np.where(valid, coordination_map, np.nan)
    high_coupling = coupling_map_clean >= thresholds['high_coupling']
    low_coordination = coordination_map_clean <= thresholds['low_coordination']
    critical_mask = high_coupling & low_coordination & valid
    
    from scipy import ndimage
    labeled, num_features = ndimage.label(critical_mask)
    sizes = ndimage.sum(critical_mask, labeled, range(1, num_features+1))
...
def create_4year_figure(data_list, output_dir='./output'):
    ...
    for col_idx, year in enumerate(years):
        ...
        coordination_classified, coord_labels, coord_props = classify_coordination(d_data, valid_mask)
        ...
        ax1.legend(handles=leg1, loc='upper center', bbox_to_anchor=(1, 1), fontsize=8, framealpha=1, facecolor='white', borderaxespad=0.05)
        ...
        ax2.legend(handles=leg2, loc='upper center', bbox_to_anchor=(1, 1), fontsize=8, framealpha=1, facecolor='white', borderaxespad=0.05)
...
def create_temporal_trend_plot(results, output_dir='./output'):
    ...
    ax2.stackplot(years, [clf_trends['Slight'], clf_trends['Low'], clf_trends['Moderate'], clf_trends['High'], clf_trends['Extreme']], labels=['Slight', 'Low', 'Moderate', 'High', 'Extreme'], colors=colors, alpha=0.8)
...
def create_bivariate_2x3_figure(data_list, output_dir='./output'):
    ...
    try:
        ...
        for idx, (year, (row, col)) in enumerate(zip(years, positions)):
            ...
            critical_mask, critical_prop = identify_critical_regions(c_data, d_data)
            
            clf_min = np.nanmin(clf_data[valid_mask])
            clf_max = np.nanmax(clf_data[valid_mask])
            if clf_max - clf_min > 1e-8:
                clf_norm = (clf_data - clf_min) / (clf_max - clf_min)
            else:
                clf_norm = np.zeros_like(clf_data)
            clf_norm = np.clip(clf_norm, 0, 1)
            
            d_min = np.nanmin(d_data[valid_mask])
            d_max = np.nanmax(d_data[valid_mask])
            if d_max - d_min > 1e-8:
                d_norm = (d_data - d_min) / (d_max - d_min)
            else:
                d_norm = np.zeros_like(d_data)
            ...
            ax.text(0.5, 0.96, f'Bivariate Map ({year})', 
        ...
        ax_stats.text(0.55, 0.9, "Spatial Pattern Statistics (2010-2022)",
        ...
        text_lines = [
            ("Dashed red boxes:", True),
            ("Critical regions", False),
            ("", False),
            ("Critical regions:", True),
            ("High coupling (C ≥ 0.6) and", False),
            ("Low coordination (D ≤ 0.4)", False),
        ]
        
        for txt, bold in text_lines:
            ax_stats.text(0.05, y_pos, txt, 
        ...
        for i in range(6):
            mask = (h_int == i)
            if i == 0:
                legend_colors[mask] = np.stack([c_vals[mask], x[mask], np.zeros_like(c_vals[mask])], axis=-1)
            elif i == 1:
                legend_colors[mask] = np.stack([x[mask], c_vals[mask], np.zeros_like(c_vals[mask])], axis=-1)
            elif i == 2:
                legend_colors[mask] = np.stack([np.zeros_like(c_vals[mask]), c_vals[mask], x[mask]], axis=-1)
            elif i == 3:
                legend_colors[mask] = np.stack([np.zeros_like(c_vals[mask]), x[mask], c_vals[mask]], axis=-1)
            elif i == 4:
                legend_colors[mask] = np.stack([x[mask], np.zeros_like(c_vals[mask]), c_vals[mask]], axis=-1)
            elif i == 5:
                legend_colors[mask] = np.stack([c_vals[mask], np.zeros_like(c_vals[mask]), x[mask]], axis=-1)
        
        legend_colors = legend_colors + m[..., np.newaxis]
        legend_colors = np.clip(legend_colors, 0, 1)
        ...
        legend_ax.text(n/2, n+1.5, 'Fragmentation →', 
                       ha='center', va='top', fontsize=8, fontfamily='Times New Roman')
        legend_ax.text(-3, n/2, 'Coordination →', 
                       ha='right', va='center', rotation=90, fontsize=8, fontfamily='Times New Roman')
        
        legend_ax.set_title('Bivariate Mapping', fontsize=9, fontweight='bold', 
                            pad=4, fontfamily='Times New Roman')
        
        legend_ax.set_xticks([0, n-1])
        legend_ax.set_xticklabels(['Low', 'High'], fontsize=7, fontfamily='Times New Roman')
        legend_ax.set_yticks([0, n-1])
        legend_ax.set_yticklabels(['High', 'Low'], fontsize=7, fontfamily='Times New Roman')
        ...
        legend_ax.text(n*0.78, n*0.22, 'Critical\nRegion', ha='center', fontsize=6, 
                       color='white', weight='bold', alpha=0.7, fontfamily='Times New Roman')
        ...
        print(f"\n❌ Bivariate plot generation failed！")
...
def main():
    ...
    try:
        ...
        result = create_bivariate_2x3_figure(all_data, output_dir)
        
        if result is not None:
            print(f"\n✅ All figures generated successfully!")
        else:
            print(f"\n⚠️ Failed to generate the bivariate figure; other figures are generated normally.")
